...
const ValidationButtonODM = ({id_odm,onRefrech,etat}) => {
 ...
 useEffect(() => {
  ...
  const handleStatusChange = async (id_odm) => {
    let newStatus;
    switch (roles) {
    ...
    axios.put(`/ODM/update/${id_odm}`, dataToSend, {
      headers: {
...
return (
  <div>
    <IconButton onClick={() => handleStatusChange(id_odm)} aria-label="Edit" size="large" id="Edit_BTN" disabled={isConsultant(roles)}>
...
export default ValidationButtonODM;